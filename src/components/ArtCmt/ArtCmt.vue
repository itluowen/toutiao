<template>
  <div :class="isShowBox1?'art-cmt-container-1':'art-cmt-container-2'">
      ...
      <div class="cmt-item" v-for="(item,i) in cmtlist" :key="i">
        ...
        <div class="cmt-header">
          ...
          <div class="cmt-header-right">
            ...
            <!-- 图标绑定点击事件处理函数，并把当前评论作为参数传递进去 -->
            <van-icon name="like-o" size="16" v-else color="gray" @click="addLike(item)" />
          </div>
        </div>
        <!-- 主体区域 -->
        <div class="cmt-body">
    ...
    <div class="cmt-box van-hairline--top" v-else>
      <textarea placeholder="友善评论、理性发言、阳光心灵" ref="iptCmt" v-model.trim="cmt" @blur="hidebox2"></textarea>
...
export default {
  ...
  methods: {
    // 初始化评论列表的数据
    async initCmtList() {
      const { data: res } = await getCmtListAPI(this.artId)
      if (res.message === 'OK') {
        // 为偏移量赋值
